    ...
    <v-card>
        <v-card-text>
            <v-row>
                ...
                <v-col cols="4" md="3">
                    <v-text-field v-model="port" label="SSH port" outlined />
                </v-col>
                <v-col cols="12" md="3">
                    <v-select
                        v-model="software"
        ...
        <v-row justify="end">
            ...
            <v-col cols="1" md="1">
                <v-btn dense :disabled="!excelFile" @click="pushExcel()"
                    >ADD</v-btn
                >
            </v-col>
        </v-row>
        ...
        <div class="pa-3" v-if="showSummary">
            ...
            <v-expansion-panels v-if="showSummary" focusable multiple>
                <v-expansion-panel v-for="(host, i) in hosts" :key="i">
                    <v-expansion-panel-header hide-actions disable-icon-rotate>
                        <div>
                            <v-icon v-if="host.success" color="success"
                                >mdi-server</v-icon
                            >
...
export default {
    ...
    methods: {
        update(items) {
            this.hostToRunCommand.push(items);
        },
        update_all(items) {
            this.hostToRunCommand = items;
        },
        importExcel() {
                        ...
                        for (const object in excelArray) {
                            ...
                            if (excelObject["host"] !== undefined) {
                                this.pushHost(
                                    excelObject["host"],
                                    excelObject["port"],
                                    excelObject["software"],
                                    excelObject["username"],
                                    excelObject["password"],
                                    excelObject["command"]
                                );
                            } else {
                                snackMessage(
        ...
        pushHost(IPorHostname, port, software, username, password, command) {
            if (
                ...
                if (
                    !this.hostToRunCommand.some(
                        (hosts) => hosts.host === IPorHostname
                    )
                ) {
                    let data = {
                        host: IPorHostname,
                    ...
                    snackMessage(
                        "Error",
                        "Host or IP address does already exist.",
                        "red"
                    );
                }
            } else {
        ...
        runCommand() {
            if (this.hostToRunCommand.length != 0) {
                ...
                let hosts = this.hostToRunCommand;
                let data = { hosts: hosts };

                axios
                    .post("/webnetter/runcommands", data)
                    .then((response) => {
                ...
                snackMessage(
                    "Error",
                    "Missing host(s) in data, try again.",
                    "red"
                );
            }
        },
